...
export interface Preferences {
  ...
  /**
   * Whether Vibeyard should install hooks and statusLine into ~/.claude/settings.json
   * (and ~/.codex/hooks.json, ~/.gemini/settings.json) on startup.
   *
   * Hooks enable live status, cost tracking, and context meter in the IDE UI, but
   * they shell out to `sh` and `/usr/bin/python3`, which fail on stock Windows
   * (Git Bash + Python 3 not in /usr/bin) and on minimal Linux/macOS environments
   * without Python 3 in /usr/bin. Default: 'disabled'. When 'disabled', any
   * previously-installed Vibeyard hooks are cleaned up on startup.
   */
  installClaudeHooks?: 'enabled' | 'disabled';
