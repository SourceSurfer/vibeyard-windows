...
export function showPreferencesModal(): void {
  ...
  let installHooksSelect: CustomSelectInstance | null = null;
  let installHooksInitialValue: 'enabled' | 'disabled' = 'disabled';
  ...
  function renderSection(section: Section) {
    ...
    if (section === 'general') {
      ...
      // Install Claude Code hooks toggle
      // Wrap in a column so we can show a long description below the row.
      const hooksWrap = document.createElement('div');
      hooksWrap.className = 'modal-toggle-field-stacked';

      const hooksRow = document.createElement('div');
      hooksRow.className = 'modal-toggle-field';

      const hooksLabel = document.createElement('label');
      hooksLabel.textContent = 'Install Claude Code hooks';

      installHooksInitialValue = appState.preferences.installClaudeHooks ?? 'disabled';
      installHooksSelect = createCustomSelect(
        'pref-install-claude-hooks',
        [
          { value: 'disabled', label: 'Disabled (recommended)' },
          { value: 'enabled', label: 'Enabled' },
        ],
        installHooksInitialValue,
      );

      hooksRow.appendChild(hooksLabel);
      hooksRow.appendChild(installHooksSelect.element);

      const hooksDesc = document.createElement('div');
      hooksDesc.className = 'modal-field-help';
      hooksDesc.textContent =
        "Hooks enable live status, cost tracking, and the context meter in Vibeyard's UI. " +
        "They shell out to /bin/sh and /usr/bin/python3, which don't exist on stock Windows " +
        "(no Git Bash or Python in /usr/bin) and on minimal Linux/macOS environments. " +
        "Disabling this keeps your global ~/.claude/settings.json clean.";

      hooksWrap.appendChild(hooksRow);
      hooksWrap.appendChild(hooksDesc);
      content.appendChild(hooksWrap);
  ...
  const save = () => {
    ...
    if (installHooksSelect) {
      const newValue = installHooksSelect.getValue() as 'enabled' | 'disabled';
      if (newValue !== installHooksInitialValue) {
        appState.setPreference('installClaudeHooks', newValue);
        // Fire main-process install/cleanup. The main side flips the persisted
        // preference too — that's intentional, the renderer copy is just a cache.
        window.vibeyard.settings.setInstallClaudeHooks(newValue).then((result) => {
          import('./alert-banner.js').then(({ showAlertBanner }) => {
            if (!result.success) {
              showAlertBanner({
                icon: '\u26A0',
                message: 'Failed to update Claude Code hooks. Check the console for details.',
              });
              return;
            }
            if (newValue === 'enabled') {
              showAlertBanner({
                icon: '\u2713',
                message: 'Claude Code hooks installed in ~/.claude/settings.json.',
              });
            } else {
              showAlertBanner({
                icon: '\u2713',
                message: 'Vibeyard hooks removed from ~/.claude/settings.json.',
              });
            }
          });
        });
  ...
  (overlay as any)._cleanup = () => {
    cleanupRecorder();
    if (defaultProviderSelect) defaultProviderSelect.destroy();
    if (installHooksSelect) installHooksSelect.destroy();
