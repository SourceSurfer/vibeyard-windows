...
function isHooksInstallEnabled(): boolean {
  try {
    return loadState().preferences.installClaudeHooks === 'enabled';
  } catch {
    return false;
  }
}
...
export function installCodexHooks(): void {
  // Gated by the global installClaudeHooks preference (shared across providers
  // because all of them use the same `# vibeyard-hook` marker and shell out to
  // sh + python3, which fail on stock Windows). When disabled, ensure existing
  // Vibeyard hooks are removed and skip installation.
  if (!isHooksInstallEnabled()) {
    cleanupCodexHooks();
    return;
  }
  ...
  const statusCmd = (event: string, status: string) =>
    `sh -c 'mkdir -p ${STATUS_DIR} && echo ${event}:${status} > ${STATUS_DIR}/$${SESSION_ID_VAR}.status ${CODEX_HOOK_MARKER}'`;

  const captureSessionIdCmd =
    `sh -c 'input=$(cat); sid=$(echo "$input" | /usr/bin/python3 -c "import sys,json; print(json.load(sys.stdin).get(\\"session_id\\",\\"\\"))" 2>/dev/null); if [ -n "$sid" ]; then mkdir -p ${STATUS_DIR} && echo "$sid" > ${STATUS_DIR}/$${SESSION_ID_VAR}.sessionid; fi ${CODEX_HOOK_MARKER}'`;
