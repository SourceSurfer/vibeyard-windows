...
export function getFullPath(): string {
  ...
  // On Windows, PATH is already populated by the OS — no need to source a login shell.
  // The login-shell trick is macOS-specific (Electron launched from Finder/Dock gets a
  // minimal PATH that misses nvm, homebrew, etc.). On Windows that problem doesn't exist.
  if (process.platform === 'win32') {
    const home = os.homedir();
    const extraDirs = [
      path.join(home, 'AppData', 'Roaming', 'npm'),
      path.join(home, 'AppData', 'Local', 'Microsoft', 'WinGet', 'Packages', 'Anthropic.ClaudeCode_Microsoft.Winget.Source_8wekyb3d8bbwe'),
      'C:\\Program Files\\nodejs',
    ];
...
export function spawnShellPty(
  ...
  // On Windows, use cmd.exe (or COMSPEC if set); on Unix use the login shell.
 ...
 *
 * On Windows, lsof and pgrep are not available; returns null immediately.
 * Future improvement: use Windows-native APIs (NtQueryInformationProcess or wmic).
 */
export function getPtyCwd(sessionId: string): Promise<string | null> {
  const instance = ptys.get(sessionId);
  if (!instance) return Promise.resolve(null);

  // lsof and pgrep are Unix-only tools; not available on Windows.
