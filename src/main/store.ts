...
/**
 * Migrate preferences for forward compatibility. Adds new fields with safe
 * defaults so older state files keep working when new preferences are introduced.
 */
function migratePreferences(state: PersistedState): void {
  if (!state.preferences) {
    state.preferences = {} as PersistedState['preferences'];
  }
  if (state.preferences.installClaudeHooks === undefined) {
    state.preferences.installClaudeHooks = 'disabled';
  }
}
