...
describe('spawnPty', () => {
  ...
  it('augments PATH with extra directories', () => {
    ...
    // On Windows getFullPath augments with Windows-native directories; on Unix with Unix dirs.
    if (process.platform === 'win32') {
      // AppData/Roaming/npm is the standard npm global bin directory on Windows
      expect(envPath).toContain('AppData');
    } else {
...
describe('getPtyCwd', () => {
  // getPtyCwd uses pgrep and lsof which are Unix-only. Pin platform to 'linux' so the
  // win32 early-return branch is skipped and the mocked execFile path is exercised.
  ...
  afterEach(() => {
    Object.defineProperty(process, 'platform', { value: savedPlatform, configurable: true, writable: true });
  });
