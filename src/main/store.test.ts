...
const DEFAULT_STATE: PersistedState = {
  ...
  preferences: { soundOnSessionWaiting: true, notificationsDesktop: true, debugMode: false, sessionHistoryEnabled: true, insightsEnabled: true, autoTitleEnabled: true, installClaudeHooks: 'disabled' },
};
...
describe('loadState', () => {
  ...
  it('parses valid JSON', () => {
    ...
    // migratePreferences fills in installClaudeHooks default
    expect(loaded).toEqual({
      ...state,
      preferences: { soundOnSessionWaiting: true, installClaudeHooks: 'disabled' },
    });
  });

  it('migrates legacy state without installClaudeHooks to disabled', () => {
    const legacyState = {
      version: 1,
      projects: [],
      activeProjectId: null,
    ...
    mockExistsSync.mockReturnValue(true);
    mockReadFileSync.mockReturnValue(JSON.stringify(legacyState));
    const loaded = loadState();
    expect(loaded.preferences.installClaudeHooks).toBe('disabled');
  });

  it('preserves explicit installClaudeHooks=enabled', () => {
    const state = {
      version: 1,
      projects: [],
      activeProjectId: null,
      preferences: { soundOnSessionWaiting: true, notificationsDesktop: true, debugMode: false, sessionHistoryEnabled: true, insightsEnabled: true, autoTitleEnabled: true, installClaudeHooks: 'enabled' as const },
    };
    mockExistsSync.mockReturnValue(true);
    mockReadFileSync.mockReturnValue(JSON.stringify(state));
    const loaded = loadState();
    expect(loaded.preferences.installClaudeHooks).toBe('enabled');
  });
