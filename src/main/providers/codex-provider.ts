...
export class CodexProvider implements CliProvider {
  ...
  reinstallSettings(): void {
    // Explicit user action — flip the global hook-install preference to enabled
    // before installing, so the gate inside installCodexHooks() does not turn
    // this into a cleanup-only no-op. Mirrors ClaudeProvider behavior.
