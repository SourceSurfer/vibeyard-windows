...
/**
 * Returns true if the user has opted in to having Vibeyard write hooks/statusLine
 * into ~/.claude/settings.json. Defaults to false (disabled).
 */
function isClaudeHooksInstallEnabled(): boolean {
  try {
    const state = loadState();
    return state.preferences.installClaudeHooks === 'enabled';
  } catch {
    return false;
  }
}
...
/**
 * Remove all Vibeyard-managed hooks (matched by `# vibeyard-hook` marker) and
 * any Vibeyard-managed statusLine from ~/.claude/settings.json. Idempotent —
 * a no-op if the file is missing, unparseable, or already clean.
 *
 * Used when the user disables `installClaudeHooks` preference, and on startup
 * to keep the global settings file clean when hooks are opt-out by default.
 */
export function cleanupVibeyardHooks(): void {
  const settingsPath = path.join(homedir(), '.claude', 'settings.json');
  let settings: Record<string, unknown>;
  try {
    settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
  ...
  // Strip Vibeyard hooks from every event type using the same logic as prepareSettings.
  ...
  if (Object.keys(existingHooks).length > 0) {
    ...
    for (const [event, matchers] of Object.entries(existingHooks)) {
      const filteredMatchers = matchers
        .map((m) => {
          ...
          const after = (m.hooks ?? []).filter((h) => !isIdeHook(h));
          if (after.length !== before) dirty = true;
    ...
    if (Object.keys(cleaned).length === 0) {
      if (settings.hooks !== undefined) dirty = true;
  ...
  // Strip Vibeyard-managed statusLine if present.
  const sl = settings.statusLine as Record<string, unknown> | undefined;
  if (sl && typeof sl === 'object' && sl.command === getStatusLineScriptPath()) {
  ...
  try {
    ...
    // Best-effort — failure to clean is non-fatal.
  }
}
 ...
 *
 * Gated by the `installClaudeHooks` preference. When disabled, this becomes a no-op
 * AND clears any previously-installed Vibeyard hooks so the global settings file
 * stays clean across upgrades.
 */
export function installHooksOnly(): void {
  if (!isClaudeHooksInstallEnabled()) {
 ...
 *
 * Gated by the `installClaudeHooks` preference. When disabled, this becomes a no-op
 * AND clears any previously-installed Vibeyard statusLine.
 */
