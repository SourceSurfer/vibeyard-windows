...
import { addMcpServer, removeMcpServer, cleanupVibeyardHooks } from './claude-cli';
import type { McpServerConfig } from './claude-cli';
import { cleanupCodexHooks } from './codex-hooks';
import { cleanupGeminiHooks } from './gemini-hooks';
import { loadState, saveState, flushState, PersistedState } from './store';
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('settings:setInstallClaudeHooks', async (_event, value: 'enabled' | 'disabled') => {
    try {
      const state = loadState();
      state.preferences.installClaudeHooks = value;
      saveState(state);
      flushState();

      if (value === 'enabled') {
        // Install hooks for every registered provider that supports hookStatus.
        const win = BrowserWindow.getAllWindows()[0] ?? null;
        for (const meta of getAllProviderMetas()) {
          if (!meta.capabilities.hookStatus) continue;
          try {
            const provider = getProvider(meta.id);
            await provider.installHooks(win);
            provider.installStatusScripts?.();
          } catch (err) {
            console.warn(`Failed to install hooks for ${meta.id}:`, err);
          }
        }
      } else {
        // Strip Vibeyard hooks from every provider's settings file.
        cleanupVibeyardHooks();
        try { cleanupCodexHooks(); } catch { /* noop */ }
        try { cleanupGeminiHooks(); } catch { /* noop */ }
      }

      return { success: true };
    } catch (err) {
      console.error('settings:setInstallClaudeHooks failed:', err);
      return { success: false };
    }
  });
