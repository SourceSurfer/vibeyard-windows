...
Uses **Vitest** with v8 coverage. Tests are co-located with source files as `*.test.ts`. Coverage HTML report outputs to `coverage/index.html`. Run a single test file: `npx vitest run src/main/store.test.ts`.
...
- **Current providers**: `ClaudeProvider`, `CodexProvider`, `GeminiProvider` — each encapsulates binary resolution, env, args, hooks, and config for its respective CLI tool.

### Shared Types

`src/shared/` contains types and constants shared across all three processes (imported by main, preload, and renderer): `types.ts` (core domain types including `SessionRecord`, `CliProviderMeta`, `ReadinessResult`), `sharing-types.ts`, and `constants.ts`.

### Other Main-Process Subsystems

- `readiness/` — Analyzes a project's AI-readiness (instructions, context, optimizations) across all active providers. Entry point: `analyzer.ts`.
- `mcp-client.ts` / `mcp-ipc-handlers.ts` — MCP inspector: connects to external MCP servers (SSE or StreamableHTTP) and exposes tool/resource/prompt listing and invocation to the renderer.
- `auto-updater.ts` — Polls GitHub releases every 4 hours via `electron-updater`; sends update events to renderer.
- `git-status.ts` / `git-watcher.ts` — Per-project git status tracking with file-watch-based refresh.
- `settings-guard.ts` — Detects and resolves conflicts when multiple providers compete to write Claude settings.
